...
MarkerClusterer.prototype.MARKER_CLUSTER_IMAGE_PATH_ = "../images/m", MarkerClusterer.prototype.MARKER_CLUSTER_IMAGE_EXTENSION_ = "png", MarkerClusterer.prototype.extend = function (t, e) {
    ...
    for (var r = 0, s = t.length, i = s; 0 !== i;) i = parseInt(i / 10, 10), r++;
    ...
    for (var r, s = !1, i = 0; r = t[i]; i++) {
        var o = this.removeMarker_(r);
        s = s || o
    }
    ...
    var n = e.fromDivPixelToLatLng(i), h = e.fromDivPixelToLatLng(o);
    return t.extend(n), t.extend(h), t
    ...
    if (!t || !e) return 0;
    var r = 6371, s = (e.lat() - t.lat()) * Math.PI / 180, i = (e.lng() - t.lng()) * Math.PI / 180,
        o = Math.sin(s / 2) * Math.sin(s / 2) + Math.cos(t.lat() * Math.PI / 180) * Math.cos(e.lat() * Math.PI / 180) * Math.sin(i / 2) * Math.sin(i / 2),
        n = 2 * Math.atan2(Math.sqrt(o), Math.sqrt(1 - o)), h = r * n;
    return h
}, MarkerClusterer.prototype.addToClosestCluster_ = function (t) {
    for (var e, r = 4e4, s = null, i = (t.getPosition(), 0); e = this.clusters_[i]; i++) {
        var o = e.getCenter();
        if (o) {
            var n = this.distanceBetweenPoints_(o, t.getPosition());
            r > n && (r = n, s = e)
        }
    }
    if (s && s.isMarkerInClusterBounds(t)) s.addMarker(t); else {
    ...
    if (i < this.minClusterSize_ && t.getMap() != this.map_ && t.setMap(this.map_), i == this.minClusterSize_) for (var o = 0; i > o; o++) this.markers_[o].setMap(null);
    ...
    if (e && e < t) for (var r, s = 0; r = this.markers_[s]; s++) r.setMap(this.map_); else {
    ...
    var s = this.textColor_ ? this.textColor_ : "black", i = this.textSize_ ? this.textSize_ : 11;
    return e.push("cursor:pointer; top:" + t.y + "px; left:" + t.x + "px; color:" + s + "; position:absolute; font-size:" + i + "px; font-family:Arial,sans-serif; font-weight:bold"), e.join("")
};
